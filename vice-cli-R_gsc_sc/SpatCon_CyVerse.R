# R script to run SpatCon via command line or RStudio, on CyVerse
# Kurt Riitters June 2024 for use in CyVerse installation only
# **********
# Input files (tif images and parameter files) are read from the data store directory "spatcon_input".
...
infile_path <- sprintf("/data-store/iplant/home/%s/spatcon_input", CyVerseUser)
if(!file.exists(infile_path)) {
  file.copy("~/spatcon/spatcon_input", home_path, recursive=TRUE)
}
