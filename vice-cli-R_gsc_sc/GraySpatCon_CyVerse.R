# R script to run GraySpatCon via command line or RStudio, on CyVerse
# Kurt Riitters June 2024 for use in CyVerse installation only
# **********
# Input files (tif images and parameter files) are read from the data store directory "grayspatcon_input".
#   If the directory does not exist, it is created with a copy of example input files
...
setwd("~/grayspatcon")
# Get the CyVerse user name and set directory names
# Use a method that works on both cloud shell and rstudio
...
# Check for input directory in data store home
# if it is not there then create it and copy the test input data
home_path <- sprintf("/data-store/iplant/home/%s", CyVerseUser)
infile_path <- sprintf("/data-store/iplant/home/%s/grayspatcon_input", CyVerseUser)
if(!file.exists(infile_path)) {
  file.copy("~/grayspatcon/grayspatcon_input", home_path, recursive=TRUE)
}
