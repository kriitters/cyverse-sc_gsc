...
#
# V1. February 2024
#   Adapted from GraySpatCon R script V4
# V2. May 2024. Adaptation for CyVerse ONLY. Changed location of 01_input, 03_output, and 05_logfiles to CyVerse
# 	user's data-store directory: /home/jovyan/data-store/home/<username>/Input_Output/SpatCon/
#   
# DISCLAIMER:
...
# V2: get the CyVerse user name and set directory names
CyVerseUser <- Sys.getenv("IPLANT_USER")
infile_path <- sprintf("/home/jovyan/data-store/home/%s/Input_Output/SpatCon/01_input", CyVerseUser)
outfile_path <- sprintf("/home/jovyan/data-store/home/%s/Input_Output/SpatCon/03_output", CyVerseUser)
logfile_path <- sprintf("/home/jovyan/data-store/home/%s/Input_Output/SpatCon/05_logfiles", CyVerseUser)
# V2: end of code block
...
#### Post-processing tif output file ####
          ...
          in_name <- tif_files[i]
          # V2. two lines changed, only one line needed
          ...
          newtif_name <- gsub(pattern = ".TIF_", replace = "_", x = newtif_name)
          # V2. direct write to data-store dumps core, try this change
