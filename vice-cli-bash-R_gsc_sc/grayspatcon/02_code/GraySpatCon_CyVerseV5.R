...
# 	user's data-store directory: /data-store/iplant/home/<username>/Input_Output/GraySpatCon/
#	Note this path is the actual mount point for data-store and should be used for scripts.
#		The symlinks in the user's home directory point to the actual mount point, however
#			the symlinks are not in the same place for all CyVerse apps.
#			In CyVerse RStudio server, the symlink is /home/rstudio/data-store/home --> /data-store/iplant/home
#			In R installed on VICE Cloud Shell is     /home/jovyan/data-store/home  --> /data-store/iplant/home
#   How to get the CyVerse user name (e.g., "kriitters") 
#		In Cloud Shell, there is an environment variable IPLANT_USER so can use CyVerseUser <- Sys.getenv("IPLANT_USER")
#		In RStudio it's not obvious (to me). There is no such environment variable.
#		ASSUMING there are only two directories in /data-store/iplant/home, i.e., "shared" and "<username>",
#			then this should work for both Cloud Shell and RStudio:
#			cmd <-sprintf("ls /data-store/iplant/home")
#			dirnames <- as.data.frame(system(cmd, intern=TRUE))
#			dirnames <- dirnames %>% filter(.[[1]] != "shared")
#			CyVerseUser <-dirnames[1,1]
#
# DISCLAIMER:
...
dirnames <- dirnames %>% filter(.[[1]] != "shared")
CyVerseUser <-dirnames[1,1]
infile_path <- sprintf("/data-store/iplant/home/%s/Input_Output/GraySpatCon/01_input", CyVerseUser)
outfile_path <- sprintf("/data-store/iplant/home/%s/Input_Output/GraySpatCon/03_output", CyVerseUser)
logfile_path <- sprintf("/data-store/iplant/home/%s/Input_Output/GraySpatCon/05_logfiles", CyVerseUser)
...
# V5. several changes to make logfile a temporary internal file, then copy to external at the end.
extlogname <- sprintf("%s/%s.log", logfile_path, now) # external path and filename
logname <- sprintf("tmplog")
#basedir <- getwd()
